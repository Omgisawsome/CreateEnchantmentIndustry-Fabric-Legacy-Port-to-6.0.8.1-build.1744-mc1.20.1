...
public class HyperExperienceOrb extends ExperienceOrb implements IEntityAdditionalSpawnData {
	...
	// FIX: Logic inlined here instead of calling the missing Fluid method
	public void applyPlayerEffects(Player player, int expAmount) {
		if (expAmount > 0) {
			// Applies Glowing effect for a duration based on XP amount (10 ticks per XP point)
			player.addEffect(new MobEffectInstance(MobEffects.GLOWING, expAmount * 10, 0));
		}
	}
